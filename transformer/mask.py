...
def create_mask(src: torch.Tensor,
    ...
    trg_mask = None
    if trg is not None:
        trg_mask = create_padding_mask(trg, trg_pad_idx)  # (256, 1, 33)
        nopeak_mask = create_nopeak_mask(trg)  # (1, 33, 33)
...
def create_nopeak_mask(seq: torch.Tensor) -> torch.Tensor:
    """
    NO PEAK MASK
    Target의 경우 그 다음 단어를 못보게 가린다
    """
    batch_size, seq_len = seq.size()
    nopeak_mask = (1 - torch.triu(torch.ones(1, seq_len, seq_len, device=seq.device), diagonal=1)).bool()
    return nopeak_mask
